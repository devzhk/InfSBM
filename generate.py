...
def load_model(config='configs/1d/edm-gs.yml'):
    ...
    net = VEPrecond(config, sigma_min=config.model.sigma_min, sigma_max=config.model.sigma_max)
...
def main():
    device = torch.device('cuda')
    net, config = load_model()
    net = net.to(device)

    init_x = torch.randn((2048, 1), device=device)
    x0 = ablation_sampler(net, init_x, num_steps=2000, 
                          sigma_min=config.model.sigma_min, sigma_max=config.model.sigma_max, 
                          schedule='ve', discretization='ve', S_churn=50, S_min=0.0001, S_max=1.0, S_noise=1.007)
    ...
    dataset = Gaussian(num_samples=config.data.num_samples,
                       mean=mean, std=std)
    data = dataset.data.flatten().numpy()
    sns.kdeplot(x0.cpu().flatten().numpy(), bw_adjust=0.9, label='VE-SDE')
    sns.kdeplot(data, bw_adjust=0.9, label='True data')
    plt.xlim((-3, 5))
    plt.legend()
    plt.savefig('figs/kde-1d.png')
